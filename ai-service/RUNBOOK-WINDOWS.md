...
## 7. แนบเรซูเม่ + OCR + พูดใส่ไมค์

ผู้ใช้แนบไฟล์เข้ามาในแชท → ระบบอ่านข้อความ → **ลบข้อมูลส่วนตัว** → สกัดทักษะ
→ บันทึกลง Supabase → น้องตรงปกถามต่อยอดจากสิ่งที่เขียนไว้จริง

### 7.1 ลำดับการอ่านไฟล์ (ทำไมถึงเรียงแบบนี้)

| ลำดับ | วิธี | ราคา | ใช้เมื่อ |
|---|---|---|---|
| 1 | text layer ของ PDF | ฟรี · ตรงเป๊ะ | export จาก Canva/Word — **ส่วนใหญ่จบตรงนี้** |
| 2 | Typhoon OCR | เสียโควตา | ไฟล์สแกน · รูปถ่าย · แคปหน้าจอ |
| 3 | Tesseract | ฟรี · ต้องลงเอง | ไม่มี API key |

ข้อ 1 ต้องมาก่อนเสมอ ถ้ายิง OCR ทุกไฟล์แบบไม่คิด โควตา 20 req/min
จะหมดตั้งแต่ทีมยังทดสอบกันไม่เสร็จ

### 7.2 ตั้งค่า

เพิ่มใน `ai-service\.env` (ระวังอย่าให้ Notepad ต่อ `.txt` ท้ายไฟล์อีก):
...
```
TYPHOON_API_KEY=sk-xxxxxxxx
TYPHOON_OCR_MODEL=typhoon-ocr
TYPHOON_OCR_MAX_PAGES=4
OCR_ENABLED=true
MAX_UPLOAD_MB=10
...
`TYPHOON_API_KEY` ใช้ตัวเดียวกับชั้น C ไม่ต้องขอใหม่
ไม่ใส่ก็ยังทำงานได้ แค่อ่านไฟล์สแกนไม่ออก

### 7.3 เพิ่มคอลัมน์ใน Supabase

```powershell
psql $env:DATABASE_URL -f db\004_resume_upload.sql
```

ไฟล์นี้ใส่ `CHECK` ที่ระดับฐานข้อมูลไว้ด้วย — ถ้าโค้ดลืม redact
Postgres จะปฏิเสธการ INSERT เลย ไม่ใช่แค่หวังว่าแอปจะทำถูก

### 7.4 เปิดเซิร์ฟเวอร์แล้วลอง

```powershell
uvicorn app.main:app --reload --port 8000
```

เช็คว่าตอนนี้ระบบอ่านไฟล์ด้วยอะไรได้บ้าง:

```powershell
curl.exe http://localhost:8000/api/resume/capabilities
```

ลองอัปโหลดจริง (เปลี่ยน path เป็นไฟล์ของตัวเอง):

```powershell
curl.exe -X POST http://localhost:8000/api/resume/upload -F "file=@C:\Users\User\Downloads\resume.pdf"
```

ดูสองอย่างในผลลัพธ์:
- `redactionSummary` — บอกว่าลบอะไรไปกี่จุด
- `redactedText` — **ต้องไม่มีชื่อ เบอร์ อีเมล ที่อยู่ เหลืออยู่**

ถ้ายังเห็นข้อมูลส่วนตัวหลุด ให้ส่งประโยคนั้นมา แล้วเพิ่มเป็นเคสใน
`tests\test_redact.py` ก่อนแก้ regex เสมอ ไม่งั้นแก้แล้วพังที่อื่นโดยไม่รู้ตัว

ลองตัวลบข้อมูลเปล่า ๆ โดยไม่ต้องมีไฟล์:

```powershell
curl.exe -X POST http://localhost:8000/api/resume/redact --data-urlencode "text=นายสมชาย ใจดี โทร 081-234-5678 GPA 3.45"
```
...
**ไม่ตั้งค่านี้ก็ยังใช้งานได้** — หน้าเว็บจะแกะ PDF ในเบราว์เซอร์เหมือนเดิม
และยังลบข้อมูลส่วนตัวให้ (`frontend/src/lib/redact.ts`) แค่อ่านไฟล์สแกนไม่ได้
และไม่บันทึกลง Supabase

### 7.6 ปุ่มไมค์

ใช้ Web Speech API ของเบราว์เซอร์ — **ไม่ต้องตั้งค่าอะไรเลย** และไม่เสียโควตา
รองรับ `th-TH` บน Chrome และ Edge

เสียงที่ถอดได้จะไปอยู่ในช่องพิมพ์ ไม่ส่งเอง เพราะการถอดเสียงไทยพลาดบ่อยพอ
ที่จะทำให้ผู้ใช้เสียโควตาคำถามไปกับประโยคที่ยังไม่ได้อ่าน

Firefox กับ Safari บนเดสก์ท็อปไม่รองรับ ถ้าจำเป็นต้องให้ใช้ได้ด้วย:

```powershell
pip install faster-whisper
```

แล้วตั้ง `ASR_PROVIDER=local_whisper` ใน `.env`
ถ้าไม่ตั้ง ปุ่มไมค์จะไม่ขึ้นบนเบราว์เซอร์พวกนั้นเลย ซึ่งดีกว่าโชว์ปุ่มที่กดแล้วพัง

### 7.7 เทสต์

```powershell
python -m pytest tests\test_redact.py tests\test_resume_pipeline.py -v
```

`test_redact.py` คือหลักฐานเดียวที่ยืนยันคำเคลม "ไม่ดูตัวตน" ของทีม
ถ้าแดง = คำเคลมนั้นไม่จริงอยู่ ณ ตอนนั้น อย่าเพิ่งขึ้นเวที
