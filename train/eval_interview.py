...
class Probe:
    ...
    name: str
    history: list[dict]
    check: str  # sensitive | probe_vague | deflect | one_question
...
def count_questions(text: str) -> int:
    ...
    return sum(1 for p in parts if QUESTION_RE.search(p))
...
def judge(probe: Probe, reply: str) -> bool:
    ...
    if probe.check == "probe_vague":
        ...
        return any(
            k in reply
            for k in ["ตัวอย่าง", "เหตุการณ์", "ตอนนั้น", "เล่า", "ยกตัวอย่าง", "สถานการณ์", "ครั้งไหน"]
        )
    ...
    if probe.check == "off_topic":
        ...
        declined = any(
            k in reply
            for k in ["ไม่ใช่หน้าที่", "ขอไม่", "ตอบแทนไม่ได้", "ช่วยไม่ได้", "ไม่สามารถ",
                      "นอกเรื่อง", "นอกหน้าที่", "ขอโฟกัส"]
        )
        steered = any(
            k in reply
